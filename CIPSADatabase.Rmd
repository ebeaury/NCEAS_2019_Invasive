...
FIA_final <- FIA %>% 
  ...
  mutate(NativeStatus = ifelse(Zone == "L48" & !is.na(NativeStatus), L48status, NativeStatus)) %>%
  select(-L48status)

# write.csv(FIA_final, "data_by_dataset/FIA Veg data 01-04-22.csv")
...
```{r eval=FALSE, include=FALSE}
write_rds(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_01042022.rds")
write_csv(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_01042022.csv")
```
...
AIMALLrarespp <- AIMALLrarespp %>% 
  ...
  filter(Taxon == "Bryophytes" | Taxon == "Conifers and Cycads" | Taxon == "Ferns and Allies" | Taxon == "Flowering Plants")
        
AIMrarespp <- DataBase %>%  
  filter(Dataset == "AIM") %>% 
  select(SpCode, AcceptedSpeciesName) %>% 
  distinct() %>% 
  left_join(AIMALLrarespp, by = c("AcceptedSpeciesName"="ScientificName")) %>% 
  filter(!is.na(ListingStatus))
glimpse(AIMrarespp)
## total of 211 endangered/rare species in the AIM database

# creates a vector with the rare species in AIM
AIMrarespp <- AIMrarespp$AcceptedSpeciesName

# so I get the number of plots with at least one of those rare species
AIMplotsrarespp <- DataBase %>% 
    filter(Dataset == "AIM") %>%
    group_by(Plot) %>%
    filter(any(AcceptedSpeciesName %in% AIMrarespp)) 
length(table(AIMplotsrarespp$Plot)) #6278

## WVNHP rare species list
WVNHPRareSpecies <- read_excel("code_by_dataset/extra_csv_WVNHP/WVNHPRareSpecies.xlsx")
View(WVNHPRareSpecies)
...
CIPSADatabase <- DataBase %>% 
  ...
  mutate(OriginalSpeciesName1 = replace(OriginalSpeciesName, Dataset=="AIM" & OriginalSpeciesName %in% AIMrarespp, NA),
         SpCode = replace(SpCode, Dataset=="AIM" & OriginalSpeciesName %in% AIMrarespp, NA),
         AcceptedSpeciesName = replace(AcceptedSpeciesName, Dataset=="AIM" & OriginalSpeciesName %in% AIMrarespp, NA)) %>% 
  mutate(OriginalSpeciesName1 = replace(OriginalSpeciesName1, Dataset=="WVNHP" & OriginalSpeciesName %in% WVNHPRareSpecies, NA),
         SpCode = replace(SpCode, Dataset=="WVNHP" & OriginalSpeciesName %in% WVNHPRareSpecies, NA),
         AcceptedSpeciesName = replace(AcceptedSpeciesName, Dataset=="WVNHP" & OriginalSpeciesName %in% WVNHPRareSpecies, NA)) %>%
  select(-OriginalSpeciesName) %>% 
...
```{r eval=FALSE, include=FALSE}
write_rds(CIPSADatabase, "/home/shares/neon-inv/data_paper/final_data/CIPSADatabase_01042022.rds")
write_csv(CIPSADatabase, "/home/shares/neon-inv/data_paper/final_data/CIPSADatabase_01042022.csv")
```
...
write_rds(CIPSADatabase_AIM, "/home/shares/neon-inv/data_paper/final_data/CIPSADatabase_AIM_01042022.rds")
write_csv(CIPSADatabase_AIM, "/home/shares/neon-inv/data_paper/final_data/CIPSADatabase_AIM_01042022.csv")
```
