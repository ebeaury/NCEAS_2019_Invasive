...
CIPSADatabase <- DataBase %>% 
  ...
  mutate(OriginalSpeciesName1 = replace(OriginalSpeciesName, Dataset=="AIM" & OriginalSpeciesName %in% AIMrarespp |
                                          Dataset=="AIM" & AcceptedSpeciesName %in% AIMrarespp, NA),
         SpCode = replace(SpCode, Dataset=="AIM" & OriginalSpeciesName %in% AIMrarespp |
                                          Dataset=="AIM" & AcceptedSpeciesName %in% AIMrarespp, NA),
         AcceptedSpeciesName1 = replace(AcceptedSpeciesName, Dataset=="AIM" & OriginalSpeciesName %in% AIMrarespp |
                                          Dataset=="AIM" & AcceptedSpeciesName %in% AIMrarespp, NA)) %>% 
...
## AIM
CIPSADatabase %>% 
  filter(Dataset=="AIM" & AcceptedSpeciesName %in% AIMrarespp)
CIPSADatabase %>% 
  filter(Dataset=="AIM" & OriginalSpeciesName %in% AIMrarespp)
## WVNHP
CIPSADatabase %>% 
...
```{r eval=FALSE, include=FALSE}
write_rds(CIPSADatabase, "/home/shares/neon-inv/data_paper/final_data/CIPSADatabase_01062022.rds")
write_csv(CIPSADatabase, "/home/shares/neon-inv/data_paper/final_data/CIPSADatabase_01062022.csv")
```
