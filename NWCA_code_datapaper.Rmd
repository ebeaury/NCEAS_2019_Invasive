...
NWCA2011resampled <- NWCA2011 %>% select(SITE_ID, VISIT_NO, DATE_COL) %>% distinct() %>% 
  group_by(SITE_ID) %>% filter(DATE_COL == max(DATE_COL)) %>%  ungroup() %>% 
  mutate(IDfilter = paste0(SITE_ID, "_", VISIT_NO))

NWCA2011 <- NWCA2011 %>%
  mutate(IDfilter = paste0(SITE_ID, "_", VISIT_NO)) %>% 
  filter(IDfilter %in% NWCA2011resampled$IDfilter) %>% 
  select(-IDfilter) %>% 
...
#finds plots that were resampled within a year and keeps only the latest survey
NWCA2016resampled <- NWCA2016 %>% select(SITE_ID, VISIT_NO, DATE_COL) %>% distinct() %>% 
  group_by(SITE_ID) %>% filter(DATE_COL == max(DATE_COL)) %>%  ungroup() %>% 
  mutate(IDfilter = paste0(SITE_ID, "_", VISIT_NO))

NWCA2016 <- NWCA2016 %>%
  mutate(IDfilter = paste0(SITE_ID, "_", VISIT_NO)) %>% 
  filter(IDfilter %in% NWCA2016resampled$IDfilter) %>% 
  select(-IDfilter) %>% 
  rename(Year = DSGN_CYCLE) %>%
  filter(!is.na(COVER)) %>% 
  #unique id matches sites across years
