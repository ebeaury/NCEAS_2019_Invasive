...
x<-NWCA2011 %>% group_by(UID, SITE_ID, PLOT) %>% add_tally() 
x<-x%>% group_by(SITE_ID) %>% count(UID)
# the issue is that SITE_ID and UID have different number of categories. For the previous data, I was giving the plots new names based on UID & PLOT, but it should have been SITE_ID & PLOT
...
glimpse(NWCA)
NWCA <- NWCA %>% 
  group_by(as.factor(Original.Site), as.factor(PLOT)) %>%
...
NWCA2011_2016_02222022 %>% distinct(UniquePlotID) %>% count()
