...
DesiredColumns <- c("Dataset", "Site", 
                    ...
                    "Original.Long", "Original.Lat", # not going to SPCIS
                    "Year", "SpCode", "AcceptedTaxonName", "Original.TaxonName", 
                    "NativeStatus", "PctCov", "FuzzedCoord", "PlotArea.m2", 
                    "Duration", "Growth.Habit", "RecordedStrata", "SamplingMethod", "Resampled"
                    )
...
```{r}
NEON <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/NEONDataPaperALLCols20220408.csv")
glimpse(NEON) 
```
...
AIM <- AIM  %>%
  ...
  mutate(PctCov = 100*prop.cover,
         FuzzedCoord = "N",
         Site = NA,
         PlotArea.m2 = ifelse(DataSet == "BLM_LMF", 3000, 10000),
         Dataset = "AIM",
...
```{r}
VEGBANK <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/All_VegBank_KPEACH_EB_LP_reduced2.csv", 
...
VEGBANK <- VEGBANK %>% 
  ...
  filter(!Original.Plot %in% PlotsRemoveWVNHP$`Plot Code`) %>% 
...
```{r}
NWCA <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/NWCA2011-2016_02222022.csv", 
                  na = c('', 'NA')) 
glimpse(NWCA)
```
...
```{r}
IL_CTAP <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/IL_CTAP_DataPaper04082022.csv", 
                 na = c('', 'NA')) 
glimpse(IL_CTAP)
```
...
IL_CTAP <- IL_CTAP %>%
  ...
  rename(Site = SiteID,
         ...
         SpCode = Accepted.Symbol,
         Year = PYear,
         # PlotArea.m2 = SampledAreaOriginal,
...
DataBase <- DataBase %>%
  ...
  select(-SpCode, -NativeStatus.x, -AcceptedTaxonName.x, -Duration.x, -Growth.Habit.x) %>% 
...
missing = DataBase %>% 
  ...
  select(Dataset, PlotArea.m2, Plot) %>% 
  distinct() %>% 
  filter(is.na(PlotArea.m2)& Dataset == "NEON")
...
DataBase <- DataBase %>% # time consuming
  # Fix issues of duplicate SpCode in AIM, IL_CTAP, NEON, VNHP 
  group_by(Dataset, Site, Plot, Original.Plot, Long, Lat, Original.Long, Original.Lat,
           Year, FuzzedCoord, RecordedStrata, PlotArea.m2, Zone, NativeStatus, AcceptedTaxonName, 
           Duration, Growth.Habit, SpCode) %>% 
...
## CVS (15), FIA (510), NPS (793) ==> observations in which OriginalSpeciesName is missing(NA) but there is no associated OriginalSpeciesNames. For FIA, all the species codes don't have any associated OriginalSpeciesName from the data I got from Jeff Corbin. For NPS, most of the species codes have associated OriginalSpeciesName in other plots, but wuite frequently associated to more than one. For CVS, is the same case as NPS, but more than one OriginalSpeciesName is frequently associated to a SpCode, so I won't fix.
...
DataBase <- DataBase %>% mutate(RecordedStrata = ifelse(is.na(RecordedStrata), "N", RecordedStrata))

DataBase %>% filter(RecordedStrata=="Y") %>% summarise(max = max(PctCov))
DataBase %>% filter(RecordedStrata=="N") %>% summarise(max = max(PctCov)) # exceeds 100%
DataBase %>% filter(RecordedStrata=="N", PctCov > 100) %>% nrow()
DataBase %>% filter(RecordedStrata=="N", PctCov > 100) %>% distinct(Dataset) # one occurrence, same genus was recorded twice in data collection
...
-   fixing site terminology

```{r}
# fixing the NAs and changing them to N [No]
DataBase <- DataBase %>% mutate(Site = ifelse(is.na(Site), "N", Site))
```
...
```{r}
DataBase <- DataBase %>%
  mutate(PlotArea.m2 = ifelse(Dataset == "IL_CTAP" | Dataset == "NEON", PlotArea.m2, PlotArea.m2),
         # NEON and FIA cover is already given on the basis of cover/m²
         SamplingMethod = ifelse(Dataset == "AIM", "Line-point intercept transect",
                                 ifelse(Dataset == "FIA", "Circular plots in a fixed pattern",
                                        ifelse(Dataset == "IL_CTAP", "Transect",
                                               ifelse(Dataset == "NPS", "Square or rectangle plot",
                                                      ifelse(Dataset == "NWCA", "Square plot", "Nested plot"))))))
...
- Are there any duplicates of plots? (I've found some for CVS, but unsure it here are more?)
```{r}

DuplicatedPlots <- DataBase %>% 
  select(Dataset, Plot, Long, Lat, Year, AcceptedTaxonName, PctCov) %>% 
  distinct() %>% 
  filter(!is.na(Long), !is.na(Lat)) %>% 
  group_by(Long, Lat, Year, AcceptedTaxonName, PctCov) %>% 
  add_tally() %>%
  filter(n > 1) %>% 
  arrange(Long, Lat, Year, AcceptedTaxonName, PctCov)

DataBaseDuplicated <- DataBase %>% 
  filter(Plot %in% DuplicatedPlots$Plot) %>% 
  group_by(Plot, Year) %>% 
  count() %>% 
  rename(numberObs = n)

DuplicatedPlots <- DuplicatedPlots %>% 
  left_join(DataBaseDuplicated) %>% 
  group_by(Long, Lat, Year, AcceptedTaxonName, PctCov) %>%
  #idea: if average number of Obs was the same as both entries given the grouping above, those plots need to be further checked
  #calculate the average number of observations within a plot across the 2 observations that share the same info
  mutate(avgObs = mean(numberObs)) %>% 
  ungroup() %>% 
  #adds a columns that if the average values is the same as the number of obs of a particular plot, adds 1, otherwise 0
  mutate(equal = ifelse(numberObs == avgObs, 1, 0)) %>% 
  group_by(Long, Lat, Year, AcceptedTaxonName, PctCov) %>%
  #sum across the observations with the same info
  # if 2 or more, plots have the same number of observations
  mutate(codeDuplicate = ifelse(sum(equal) >1, 1, 0)) %>% 
  filter(codeDuplicate == 1) %>% 
  group_by(Long, Lat, Year, AcceptedTaxonName, PctCov) %>%
  mutate(NewPlotID = cur_group_id()) %>% ungroup()
  
DataBaseDuplicated2 <- DataBase %>% 
  filter(Plot %in% DuplicatedPlots$Plot) %>% 
  left_join(select(DuplicatedPlots, Plot, NewPlotID)) %>% 
  arrange(NewPlotID, Plot, Long, Lat, Year, AcceptedTaxonName, PctCov) %>% 
  group_by(NewPlotID) %>% 
  add_count(AcceptedTaxonName, PctCov) 

#plots that are not all the same observations
DataBaseDuplicated3 <- DataBaseDuplicated2 %>% 
  filter(any(n == 1)) 

# plots in which all observations are equal
DataBaseDuplicated4 <- DataBaseDuplicated2 %>% 
  filter(!NewPlotID %in% DataBaseDuplicated3$NewPlotID) %>% 
  select(Plot, NewPlotID) %>% 
  distinct() %>% 
  left_join(select(DuplicatedPlots, c(Plot, numberObs, Dataset))) %>% 
  ungroup() %>% 
  select(-NewPlotID) %>% 
  distinct() %>% 
  arrange(Dataset, numberObs)
#It seems that for NWCA the plots have very low richness, and those species dominate, so the obs should be right
#but for NPS and VNHP, those plots seems incorrect
  write_csv("/home/shares/neon-inv/data_paper/datachecks/PlotDuplicates.csv")
```

- fix ALFL plot names from for example "ALFL_ALFL.48" to "ALFL.48"
```{r}
DataBase %>% filter(Site == "ALFL") %>% select(Plot) %>% distinct() #identifies the plots with wrong spelling

DataBase <- DataBase %>% 
  mutate(Plot = ifelse(Plot=="ALFL_ALFL.142", "ALFL.142",
                ifelse(Plot=="ALFL_ALFL.47", "ALFL.47",
                ifelse(Plot=="ALFL_ALFL.48", "ALFL.48",
                ifelse(Plot=="ALFL_ALFL.49", "ALFL.49",
                ifelse(Plot=="ALFL_ALFL.50", "ALFL.50",
                ifelse(Plot=="ALFL_ALFL.51", "ALFL.51", Plot))))))) #fixes

DataBase %>% filter(Site == "ALFL") %>% select(Plot) %>% distinct() #no more errors
```
...
```{r eval=FALSE, include=FALSE}
write_rds(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_04122022.rds")
write_csv(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_04122022.csv")
```
...
DesiredColumns2 <- c("Dataset", "Site", 
                    ...
                    "PctCov", "RecordedStrata", "PlotArea.m2", "SamplingMethod", "Resampled"
                    )
...
# write.csv(ListRareSpecies, "final_data/ListRareSpecies_04092022.csv", row.names = F)
```
...
SPCIS_plant_taxa <- SPCISDatabase %>% 
  select(Dataset, Plot, Year, SpCode, AcceptedTaxonName, Original.TaxonName, 
         NativeStatus, Duration, GrowthHabit, PctCov, RecordedStrata) %>% 
  as_tibble()

SPCIS_plots <- SPCISDatabase %>% 
  select(Plot, Year, Original.Plot, Site, Long, Lat, FuzzedCoord, Zone, PlotArea.m2, SamplingMethod, Resampled) %>%
  # FIA_2099 plot was duplicated even after using the distinct() function 
  # the issue was on the decimals of the plot area column
  mutate(PlotArea.m2 = ifelse(Plot == "FIA_2099", 673.3363, PlotArea.m2)) %>% 
    distinct() %>% 
  as_tibble()
...
```{r}
#plant data
write_rds(SPCIS_plant_taxa, "/home/shares/neon-inv/data_paper/final_data/SPCIS_plant_taxa_0412022.rds")
write_csv(SPCIS_plant_taxa, "/home/shares/neon-inv/data_paper/final_data/SPCIS_plant_taxa_04122022.csv")

#plot data
write_rds(SPCIS_plots, "/home/shares/neon-inv/data_paper/final_data/SPCIS_plots_0412022.rds")
write_csv(SPCIS_plots, "/home/shares/neon-inv/data_paper/final_data/SPCIS_plots_04122022.csv")
```
...
ggsave("GraphInvadedPlotsperDataset02282022.png", dpi=300)
```
...
graph1<-SPCISDatabaseEco %>% filter(NativeStatus == status,
...
# GREAT PLAINS
graph2<-SPCISDatabaseEco %>% filter(NativeStatus == status,
...
graph3<-SPCISDatabaseEco %>% filter(NativeStatus == status,
...
graph4<-SPCISDatabaseEco %>% filter(NativeStatus == status,
...
graph5<-SPCISDatabaseEco %>% filter(NativeStatus == status,
...
# NORTHERN FORESTS
graph6<-SPCISDatabaseEco %>% filter(NativeStatus == status,
...
graph7<-SPCISDatabaseEco %>% filter(NativeStatus == status,
...
graph8<-SPCISDatabaseEco %>% filter(NativeStatus == status,
...
# TEMPERATE SIERRAS
graph9<-SPCISDatabaseEco %>% filter(NativeStatus == status,
...
png('figures/ecoregionsNN/graphcomginedNN020282022.png', width = 1920,height = 1080)
print(all)
dev.off()
...
png('figures/ecoregionsN/graphcomginedN02282022.png', width = 1920,height = 1080)
print(all)
dev.off()
