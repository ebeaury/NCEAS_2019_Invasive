...
DesiredColumns <- c("Dataset", "Site", 
                    ...
                    "NativeStatus", "Original.PctCov", "PctCov", "FuzzedCoord", "PlotArea.m2", "SampledArea", 
...
WVNHPStrata <- read.csv("/home/shares/neon-inv/raw_VegBank_data/WV_species_data.csv") %>% 
  select(Plot.Code, Scientific.Name, Stratum, Real.Cover) %>% 
  group_by(Plot.Code, Scientific.Name) %>% 
  add_tally() %>% 
  summarise(Real.Cover = sum(Real.Cover)) %>% 
  mutate(RecordedStrata = ifelse(n == 1, "N", "Y")) %>% 
  ungroup()

WVNHPStrata %>% 
  filter(Real.Cover>100) %>% 
  distinct(RecordedStrata)
```
...
DataBase <- DataBase %>% # time consuming
  ...
  group_by(Dataset, Site, Plot, Original.Plot, Long, Lat, Original.Long, Original.Lat,
           Year, FuzzedCoord, SampledArea, RecordedStrata, PlotArea.m2, Zone, NativeStatus, AcceptedTaxonName, 
...
+ CONTINUE HERE fixing RecordedStrata terminology
```{r}
...
DataBase <- DataBase %>%
  group_by(Plot, Year) %>%
  add_tally() %>% 
  mutate(Resampled = ifelse(n>1, "Y", "N"))

## CHECK whether the code is working as expected
```
...
```{r}
DataBase <- DataBase %>%
  mutate(PlotArea.m2 = ifelse(Dataset == "IL_CTAP" | Dataset == "NEON", PlotArea.m2, SampledArea),
...
```{r eval=FALSE, include=FALSE}
write_rds(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_03152022.rds")
write_csv(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_03152022.csv")
```
...
DesiredColumns2 <- c("Dataset", "Site", 
                    ...
                    "PctCov", "RecordedStrata", "PlotArea.m2", "SamplingMethod"
                    )
...
```{r eval=FALSE, include=FALSE}
write_rds(SPCISDatabase, "/home/shares/neon-inv/data_paper/final_data/SPCISDatabase_03152022.rds")
write_csv(SPCISDatabase, "/home/shares/neon-inv/data_paper/final_data/SPCISDatabase_03152022.csv")
```
