...
## NEXT STEPS:
# filter the OriginalSpeciesName with sp. or spp. by the end --> create a new object
# remove the "ceae sp."
...
NEON <- NEON %>%
  ...
  rename(Dataset = dataset,
         ...
         Plot = plotID,
         Year = year,
         AcceptedSpeciesName = AccSpeciesName,
...
DataBase <- DataBase %>% 
  mutate(#fixes error in VNHP
         AcceptedSpeciesName = ifelse(OriginalSpeciesName == "Galium circaezans" & SpCode=="GABO2", 
                                      "Galium circaezans", AcceptedSpeciesName),
         SpCode = ifelse(OriginalSpeciesName == "Galium circaezans" & SpCode=="GABO2", "GACI2", SpCode),
         
         #Fixes errors in NPS
         AcceptedSpeciesName = ifelse(OriginalSpeciesName == "Galium circaezans" & SpCode=="GACIC", 
                                      "Galium circaezans var. circaezans", AcceptedSpeciesName),
         
         AcceptedSpeciesName = ifelse(OriginalSpeciesName == "Galium circaezans" & SpCode=="GACIH", 
                                      "Galium circaezans var. hypomalacum", AcceptedSpeciesName),
         
         AcceptedSpeciesName = ifelse(OriginalSpeciesName == "Euthamia graminifolia" & SpCode=="EUCA26", 
                                      "Euthamia graminifolia", AcceptedSpeciesName),
         SpCode = ifelse(OriginalSpeciesName == "Euthamia graminifolia" & SpCode=="EUCA26", "EUGR5", SpCode),
         
         #fixes error in NEON
         AcceptedSpeciesName = ifelse(OriginalSpeciesName == "Eupatorium capillifolium (Lam.) Small" & SpCode=="EUCA26", 
                                      "Eupatorium capillifolium", AcceptedSpeciesName),
         SpCode = ifelse(OriginalSpeciesName == "Eupatorium capillifolium (Lam.) Small" & SpCode=="EUCA26", "EUCA5", SpCode)) 

DataBase <- DataBase %>% # time consuming
  # Fix issues of duplicate SpCode in AIM, IL_CTAP, NEON, VNHP 
  group_by(Dataset, Site, Plot, OriginalPlot, Long, Lat, Original.Long, Original.Lat,
           Year, FuzzedCoord, SampledArea, Zone, NativeStatus, AcceptedSpeciesName, 
           Duration, Growth.Habit, SpCode) %>% 
  summarize(PctCov = ifelse(!is.na(SpCode), sum(PctCov, na.rm=TRUE), PctCov), # this is the column I need to sum
...
+ NA for OriginalSpeciesName
```{r}
MissingOrigName<-DataBase %>% 
  filter(is.na(OriginalSpeciesName)) %>% 
  select(Dataset, OriginalSpeciesName, SpCode) %>% 
  group_by(Dataset, OriginalSpeciesName, SpCode) %>% 
  summarise(n_obs = n())
...
```{r eval=FALSE, include=FALSE}
# write_rds(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_01252022.rds")
# write_csv(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_01252022.csv")
```
...
```{r eval=FALSE, include=FALSE}
write_rds(CIPSADatabase, "/home/shares/neon-inv/data_paper/final_data/CIPSADatabase_01252022.rds")
write_csv(CIPSADatabase, "/home/shares/neon-inv/data_paper/final_data/CIPSADatabase_01252022.csv")
```
...
write_rds(CIPSADatabase_AIM, "/home/shares/neon-inv/data_paper/final_data/CIPSADatabase_AIM_01252022.rds")
write_csv(CIPSADatabase_AIM, "/home/shares/neon-inv/data_paper/final_data/CIPSADatabase_AIM_01252022.csv")
```
