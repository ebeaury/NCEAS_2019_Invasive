...
## NEXT STEPS:
# filter the Original.TaxonName with sp. or spp. by the end --> create a new object
# remove the "ceae sp."
# separate the genus from sp/spp, keep the orginal column
# find the USDA species codes for them based on tax
# left_join the new SpCodes and mutate the "old" SpCode column to keep the new match
...
NEON <- NEON %>%
  mutate(FuzzedCoord = "N",
         ...
         ## area sampled needs to be fixed in the original data, so then we can have the original sampled area
         SampledArea = 1) %>%
...
Imports the file
```{r}
NWCA <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/NWCA2011-2016_01242022.csv", 
                  na = c('', 'NA')) 
glimpse(NWCA)
```
...
Imports the file
```{r}
IL_CTAP <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/IL_CTAP_DataPaper01202022.csv", 
                 na = c('', 'NA')) 
glimpse(IL_CTAP)
```
...
DataBase <- DataBase %>%
  mutate(Original.PctCov = PctCov,
         PctCov = ifelse(Dataset == "NEON", PctCov, PctCov/SampledArea),
         Original.SampledArea = ifelse(Dataset == "IL_CTAP", Original.SampledArea, SampledArea),
         SampledArea = 1)
...
DesiredColumns2 <- c("Dataset", "Site", 
                    "Plot", "Original.Plot", "Long", "Lat","FuzzedCoord", 
                    "Year", "SpCode", "AcceptedTaxonName", "Original.TaxonName", 
                    "NativeStatus", "Duration", "GrowthHabit",
                    "PctCov", "Original.SampledArea", "Zone" 
                    )
