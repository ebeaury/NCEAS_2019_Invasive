...
Imports the file
```{r}
IL_CTAP <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/IL_CTAP_DataPaper02242022.csv", 
                 na = c('', 'NA')) 
glimpse(IL_CTAP)
```
...
DataBase %>% 
  ...
  count(Plot) %>%
  nrow()
## 5005 plots from NPS and WVNHP with zeros.
...
DataBase <- DataBase %>% # time consuming
  ...
  group_by(Dataset, Site, Plot, Original.Plot, Long, Lat, Original.Long, Original.Lat,
           Year, FuzzedCoord, SampledArea, RecordedStrata, Original.SampledArea, Zone, NativeStatus, AcceptedTaxonName, 
...
```{r}
DataBase <- DataBase %>%
  mutate(Original.SampledArea = ifelse(Dataset == "IL_CTAP" | Dataset == "NEON", Original.SampledArea, SampledArea),
...
# min and max values of cover per dataset
# I wanna make sure I am dividing by sampled area to get %/m² for the datasets that this is not done before grouping in the database
DataBase %>%
  group_by(Dataset) %>%
  mutate(
...
```{r}
remove<-"SampledArea"
DesiredColumns <- setdiff(DesiredColumns, remove)
