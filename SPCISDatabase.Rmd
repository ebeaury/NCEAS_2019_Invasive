...
DesiredColumns <- c("Dataset", "Site", 
                    ...
                    "NativeStatus", "Original.PctCov", "PctCov", "FuzzedCoord", "Original.SampledArea", "SampledArea", 
                    "Duration", "Growth.Habit", "RecordedStrata", "SamplingMethod"
                    )
...
Imports the file
```{r}
NEON <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/NEONDataPaperALLCols20220223.csv")
glimpse(NEON) 
```
...
AIM <- AIM  %>%
  ...
  mutate(PctCov = 100*prop.cover,
         FuzzedCoord = "N",
         Site = NA,
         SampledArea = ifelse(DataSet == "BLM_LMF", 3000, 10000),
         Dataset = "AIM",
...
NPS <- NPS %>%
  mutate(Original.Plot = as.character(Plot),
         ...
         Original.Lat = Lat,
         Original.Long = Long,
         SampledArea = as.double(Area_sampled)) %>% 
  select(-Plot) %>%
...
CVSresampled2 <- CVSresampled %>%
  select(`Author Observation Code`, previousObsCode, NewPlotID) %>% 
...
WVNHPissues <- WVNHP %>% 
  ...
  write_csv("/home/shares/neon-inv/data_paper/code_by_dataset/extra_csv_WVNHP/WVNHPPlotVerify.csv")
```
...
Imports the file
```{r}
IL_CTAP <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/IL_CTAP_DataPaper02222022.csv", 
                 na = c('', 'NA')) 
glimpse(IL_CTAP)
```
...
IL_CTAP <- IL_CTAP %>%
  ...
  rename(Site = SiteID,
         ...
         SpCode = Accepted.Symbol,
         Year = PYear,
         Original.SampledArea = SampledAreaOriginal,
...
*Option2*: Uses SpCode from the original dataset and replaces the NA from Accepted.Symbol2 (PLANTS codes)
LP: It seems to me that this is not the way to go. The "NOMATCH" codes are for both species that USDA PLANTS don't have but was identified by researchers at the source [mainly CVS, WVNHP, VNHP] or clearly stated as Unknown [in CVS it's {Unkown}]
```{r eval=FALSE, include=FALSE}
...
## they differ. Need to fix this
DataBase <- DataBase %>% 
  mutate(FuzzedCoord = ifelse(is.na(Lat) & is.na(Long), "Missing", FuzzedCoord))
table(DataBase$FuzzedCoord)
table(DataBase$Zone)
##FIXED!
...
DataBase <- DataBase %>% # time consuming
  ...
  group_by(Dataset, Site, Plot, Original.Plot, Long, Lat, Original.Long, Original.Lat,
           Year, FuzzedCoord, SampledArea, Zone, NativeStatus, AcceptedTaxonName, 
...
```{r}
duplicates <- DataBase %>% # time consuming
  group_by(Dataset, Plot, Original.TaxonName, Year) %>% 
  filter(!is.na(Original.TaxonName)) %>% 
  summarise(n_obs = n()) %>% 
  filter(n_obs > 1)
table(duplicates$Dataset)

# RECHECK THIS, AND MAKE SURE THESE ARE NOT SHOWING UP ANYMORE
# these duplicates are already present in the original datasets, assuming the ones who Id them that each entry is a different morphotype, and potentially a distinct species
```
...
+ creating a Pct Cover column %/m² and duplicating the cover column to Original.
```{r}
DataBase <- DataBase %>%
  mutate(Original.PctCov = PctCov,
         # NEON cover is already given on the basis of cover/m²
         PctCov = ifelse(Dataset == "NEON", PctCov, 
                         #Some datasets (NPS & XX) plots we couldn't retrieve the SampledArea (there were 0, I changed to NA as 0 does not make sense)
                         #for these plots, I am keeping the cover as they are, and I point that out in the paper description
                         ifelse(is.na(Original.SampledArea), PctCov, PctCov/SampledArea)),
         ## check if only for IL_CTAP I have a original sampled area column. I think so
         Original.SampledArea = ifelse(Dataset == "IL_CTAP" | Dataset == "NEON", Original.SampledArea, SampledArea),
         SampledArea = 1,
         SamplingMethod = ifelse(Dataset == "AIM", "Line-point intercept transect",
                                 ifelse(Dataset == "FIA", "Circular plots in a fixed pattern",
                                        ifelse(Dataset == "IL_CTAP", "Transect",
                                               ifelse(Dataset == "NPS", "Square or retangle plot",
...
DataBase %>% 
  filter(is.na(Original.SampledArea)) %>% 
  select(Dataset, Plot) %>% 
  disitnct() %>% 
  count(Plot)
...
```{r eval=FALSE, include=FALSE}
# write_rds(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_02172022.rds")
# write_csv(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_02172022.csv")
```
...
DesiredColumns2 <- c("Dataset", "Site", 
                    ...
                    "PctCov", "RecordedStrata", "Original.SampledArea", "SamplingMethod"
                    )
...
```{r eval=FALSE, include=FALSE}
# write_rds(SPCISDatabase, "/home/shares/neon-inv/data_paper/final_data/SPCISDatabase_02172022.rds")
# write_csv(SPCISDatabase, "/home/shares/neon-inv/data_paper/final_data/SPCISDatabase_02172022.csv")
```
...
ggsave("GraphInvadedPlotsperDataset02012022.png", dpi=300)
```
...
x<- SPCISDatabase %>%
  ...
  filter(n>1) 
unique(x$Dataset)
dim(x) ## okay, but let me confirm in te next code chunck based on the Original Lat/Long

xx<- DataBase %>%
  ungroup() %>%
  select(Dataset, Original.Lat, Original.Long, Year) %>%
  distinct() %>%
  group_by(Dataset, Original.Lat, Original.Long) %>% 
  # I'm unsure why, but my summarise is not working properly. There are resampled 
  # CVS plots that are not being grouped, even though they have the same coordinates
  dplyr::summarise(n_obs = n()) %>% 
  filter(n_obs>1) %>% 
  filter(!is.na(Original.Lat)) %>% 
  #LP: talked to Eve, and it's a precision issue. Should not worry about it
  filter(Dataset != "NPS") %>% 
  # it's correct, confirmed in the original data
  filter(Dataset != "IL_CTAP",
         Dataset != "AIM") %>% 
  # it's correct, the renaming of resampled plots was done based on the aggregate function Jeff C built
  filter(Dataset != "FIA") %>%  
  left_join(distinct(select(DataBase, c("Original.Lat", "Original.Long","Year","Plot","Original.Plot"))))
dim(xx)
unique(xx$Dataset)
