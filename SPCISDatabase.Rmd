...
Plot_shape_WV <- read_csv("/home/shares/neon-inv/raw_VegBank_data/WV_plot_data.csv")

Plot_shape_WV <- Plot_shape_WV %>%  select(`Plot Code`, `Plot Shape`) %>% distinct() %>% filter(`Plot Code` %in% WVNHPPlots$UniqueID) %>% 
...
VEGBANK <- VEGBANK %>% 
  ...
  filter(Dataset == "WVNHP" & grepl("^NWCA", Original.Plot)) %>% 
  # adds
...
```{r}
NWCA <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/NWCA2011-2016_05182022.csv", 
                  na = c('', 'NA')) 
glimpse(NWCA)
```
...
#previous file - observations per dataset
FULLDatabase_04182022 <- read_csv("archived/FULLDatabase_04182022.csv")
x<-FULLDatabase_04182022 %>% 
  count(Dataset) %>% 
  rename(nfull = n,
         datsetfull = Dataset)

#with updated NWCA info
DataBase %>% 
  count(Dataset)%>% 
  cbind(x) %>% 
  mutate(ndif = n-nfull)

#CVS has 310 more obs
#NEON has 1 more observation
#NWCA has 15701 less observations 
#WNHP has 5 more observations

## Besides NWCA, the discrepancy on the number of obs is because the filter of pctcov >0 happens a few chunks below. Those extra bservations are due to pctcover == 0

#previous file - plots per dataset
x<-FULLDatabase_04182022 %>% 
  group_by(Dataset) %>% 
  distinct(Plot) %>% 
  count()
...
DataBase %>% 
  ...
  cbind(x) %>% 
  mutate(n = n...2-n...4)
# all datasets have the same number of plots, besides NWCA. NWCA has less 949 plots - OKay as I fixed the plot overestimation

## why is CVS with more obs than before? Yes, because those plots have observations with zeros for pctcover and they are later on removed in this code 

cvsfull<- FULLDatabase_04182022 %>% filter(Dataset == "CVS") %>% count(Plot)
cvsnew<-DataBase %>% filter(Dataset == "CVS") %>% count(Plot) %>% rename(Plotnew = Plot, nNew = n) %>% 
  cbind(cvsfull) %>% mutate(ndif = nNew-n) %>% filter(ndif >0) %>% arrange(desc(ndif))
...
DataBase <- DataBase %>% mutate(Site = ifelse(is.na(Site), "N", Site),
                                Original.Site = ifelse(is.na(Original.Site), "N", Original.Site))
```

-  FIX THIS!!!!!!!! creating a Sampling method column.
...
Checking whether the number of observations is the same across datasets (not apply to NWCA, which I fixed it)
```{r}
x<-FULLDatabase_04182022 %>% 
  count(Dataset) %>% 
  rename(nfull = n,
...
```{r eval=FALSE, include=FALSE}
write_rds(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_04202122.rds")
write_csv(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_04202122.csv")
```
...
```{r eval=FALSE, include=FALSE}
#plant data
write_rds(SPCIS_plant_taxa, "/home/shares/neon-inv/data_paper/final_data/SPCIS_plant_taxa_0421022.rds")
write_csv(SPCIS_plant_taxa, "/home/shares/neon-inv/data_paper/final_data/SPCIS_plant_taxa_04212022.csv")

#plot data
write_rds(SPCIS_plots, "/home/shares/neon-inv/data_paper/final_data/SPCIS_plots_04212022.rds")
write_csv(SPCIS_plots, "/home/shares/neon-inv/data_paper/final_data/SPCIS_plots_04212022.csv")
```
...
ggsave("/home/shares/neon-inv/data_paper/figures/GraphInvadedPlotsperDataset04212022.png", dpi=300)
```
...
png('/home/shares/neon-inv/data_paper/figures/ecoregionsNN/graphcombinedIntroduced04212022.png', width = 1920,height = 1080)
print(all)
dev.off()
...
png('/home/shares/neon-inv/data_paper/figures/ecoregionsN/graphcombinedNat4212022.png', width = 1920,height = 1080)
print(all)
dev.off()
