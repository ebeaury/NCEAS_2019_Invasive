...
DesiredColumns <- c("Dataset", "Site", "Original.Site",
                    "Plot", "Original.Plot", "Long", "Lat", "Zone", "EcoRegionLevelI",
                    ...
                    "Duration", "Growth.Habit", "RecordedStrata", "NumberOfStrata", "PlotArea.m2", "SamplingMethod", "Resampled"
                    )
...
```{r}
NEON <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/NEONDataPaperALLCols09142022.csv")
glimpse(NEON) 
```
...
# given that the WREF_07* plots year comes from vst, but the data is mixed between 2016 (div) ad 2017 (vst), I am using the month for 2016 div
NEON <- NEON %>% 
  mutate(Month=ifelse(is.na(Month), 7, Month))
```
...
NEON <- NEON %>%
  mutate(FuzzedCoord = "N",
         ...
         # it's 1 because the %cover is on the m² basis.
         # SampledArea = 1,
...
Imports file with plots to be excluded (same plots are in CVS)

```{r}
NPSexclude2 <- read.csv("/home/shares/neon-inv/data_paper/code_by_dataset/extra_csv_NPS/CVS_NPS_DuplicatedPlots.csv", 
                na = c('', 'NA'))
glimpse(NPSexclude2)
```

Imports file with plots to be excluded (same plots are in WVNHP)

```{r}
NPSexclude3 <- read.csv("/home/shares/neon-inv/data_paper/code_by_dataset/extra_csv_NPS/WVNHP_NPS_DuplicatedPlots.csv", 
                na = c('', 'NA'))
glimpse(NPSexclude3)
```

Imports file with PctCov_100 and NumberOfStrata

```{r}
NPS100 <- read.csv("/home/shares/neon-inv/data_paper/code_by_dataset/extra_csv_NPS/NPSPct_100_Sept22.csv")
```
...
```{r}
NPS <- NPS %>%
  #adds number of strata and PctCov_100
  left_join(NPS100, by = c("UniqueID"="Plot", "Species"="SpCode.Original")) %>% 
  #removes plots that are also found in VANHP (NPS plots in Virginia)
  filter(!UniqueID %in% NPSexclude$Plot) %>% 
  #removes plots that are also found in CVS (NPS plots in CVS)
  filter(!UniqueID %in% NPSexclude2$UniqueID) %>%
    #removes plots that are also found in WVNHP (NPS plots in CVS)
  filter(!UniqueID %in% NPSexclude3$UniqueID_NPS) %>%
  filter(UniqueID != "GUCO_1401") %>% #dropping plot as a suggestion from Bob Peet (the plots should have other 13 species with no cover associated to them. Given that this info cannot be retrieved at this point I am removing it)
  mutate(
  ...
  #fixing original plot ID and plotID for NPS plots
  mutate(Original.Plot=ifelse(Site=="ALFL" | Site=="GLAC" | Site=="NERI" | Site== "GARI", Plot,paste0(Site,".",Plot)),
         Plot=Original.Plot) %>% 
...
Creating file with old and new NPS codes
```{r}
NPScodes <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/NPS_DataPaper_27July2021.csv", 
                na = c('', 'NA'))
glimpse(NPScodes)

NPScodes <- NPScodes %>% 
  select(Site, Plot, UniqueID) %>% distinct() %>% 
  mutate(New.PlotID=ifelse(Site=="ALFL" | Site=="GLAC" | Site=="NERI" | Site== "GARI", Plot,paste0(Site,".",Plot))) %>%
  select(-Site, -Plot)
write.csv(NPScodes, "/home/shares/neon-inv/data_paper/code_by_dataset/extra_csv_NPS/NPScodes.csv", row.names = F)
```
...
Imports file with number of strata and pctcov_100
...
CVSresampled <- rbind(CVS_plot_data_VA, CVS_plot_data_AK, CVS_plot_data_AL, CVS_plot_data_FL, 
                      ...
                      CVS_plot_data_TN, CVS_plot_data_TX) %>% distinct()

previousObsCode <- CVSresampled %>% select(previousObsCode, `Author Observation Code`, `Observation Start Date`)

CVSresampled2 <- CVSresampled %>% #plots resampled 2x
  left_join(previousObsCode, by = c(`Author Observation Code` = "previousObsCode")) %>% # plots resampled 3x
  left_join(previousObsCode, by = c(`Author Observation Code.y` = "previousObsCode")) %>%  # plots resampled 4x
  ...
  mutate(SameYear = ifelse(lubridate::year(`Observation Start Date.x`) == lubridate::year(Date_prev), 1, 0),
         LaterMonth = ifelse(SameYear ==1 & month(`Observation Start Date.x`) > month(Date_prev), 1,
                             ifelse(SameYear ==0, NA, 0)),
         PlotsRemove  = ifelse(LaterMonth == 1, previousObsCode, `Author Observation Code.x`)) 

CVSresampled3 <- CVSresampled2 %>% filter(SameYear==0) %>% 
  # removing duplicated lines with partial plot match (i.e., not all years in a single row)
  filter(!`Author Observation Code.x` %in% `Author Observation Code.y`) %>% 
  filter(!`previousObsCode` %in% `Author Observation Code.y`) %>% 
  #create new id for plots
  group_by(Source) %>% 
  mutate(NewPlotID = ifelse(SameYear==0, paste0("CVS_", Source, row_number()), NA)) %>% 
  ungroup()

CVSresampled4 <- CVSresampled3 %>% select(`Author Observation Code.x`, NewPlotID) %>% rename(Original.Plot=`Author Observation Code.x`)
CVSresampled5 <- CVSresampled3 %>% select(`Author Observation Code.y`, NewPlotID) %>% rename(Original.Plot=`Author Observation Code.y`)
CVSresampled6 <- CVSresampled3 %>% select(`Author Observation Code.y.y`, NewPlotID) %>% rename(Original.Plot=`Author Observation Code.y.y`)
CVSresampled7 <- CVSresampled3 %>% select(`previousObsCode`, NewPlotID) %>% rename(Original.Plot=`previousObsCode`)

#list of plots resampled [in different years]
CVSresampledFINAL <- CVSresampled4 %>% rbind(CVSresampled5) %>% rbind(CVSresampled6) %>% rbind(CVSresampled7) %>% filter(!is.na(Original.Plot))


#list of plots sampled twice in the same year. These ones were sampled earlier in the season.
CVSRemove <- CVSresampled2 %>% 
  select(PlotsRemove) %>% 
  filter(!is.na(PlotsRemove))

glimpse(CVSRemove)
glimpse(CVSresampledFINAL)
```

Finding duplicated plots within the CVS data

```{r}
# 734 plots are present in both datasheets of NCFringe and NCCoastal
...
# 58 plots are present in both datasheets of NCPP and Proj129
# and they contain the same species and associated information
...
VEGBANK <- VEGBANK %>% 
  ...
  left_join(CVSresampledFINAL, by = "Original.Plot") %>% 
  ...
  mutate(Plot = ifelse(is.na(NewPlot), Plot, NewPlot)) %>%
...
```{r}
IL_CTAP <- read.csv("/home/shares/neon-inv/data_paper/data_by_dataset/IL_CTAP_DataPaper09202022.csv", 
                 na = c('', 'NA')) 
glimpse(IL_CTAP)
```

Imports file to fix month issues
```{r}
CTAPdateProblems_fromLais_R <- read_csv("/home/shares/neon-inv/data_paper/code_by_dataset/extra_csv_CTAP/CTAPdateProblems_fromLais.R.csv") %>% 
  filter(SPCIS_status=="no") %>%  select(-...1, -n_obs, -SPCIS_status, -comment)
glimpse(CTAPdateProblems_fromLais_R)
```
...
```{r}
IL_CTAP <- IL_CTAP %>%
  left_join(CTAPdateProblems_fromLais_R) %>% 
  mutate(PMonth = ifelse(is.na(NEW_MONTH), PMonth, NEW_MONTH),
         FuzzedCoord = "Y",
         Original.Lat = Lat,
         Original.Long = Long,
         Lat = round(Lat, 2),
...
DataBase %>% 
  filter(is.na(Year)) %>% 
  nrow() # missing Year = 65232 records 

DataBase %>% 
  filter(is.na(Year)) %>% 
  group_by(Dataset) %>% 
  tally()# missing Year = all 65232 records are from NPS
...
DataBase %>%
  ...
  distinct() %>%
  count(Zone) # none
table(DataBase$Zone, useNA = "ifany") #none, as zone was associated depending on the geographic region of a particular dataset
...
DataBase %>% 
  ...
  group_by(Dataset) %>%
  count() #number of plots with missing plot area info
...
# fixes PctCov_100 for the datasets that we did not aggregated strata
DataBase <- DataBase %>% 
  mutate(PctCov_100 = ifelse(Dataset=='AIM'|Dataset=='CVS'|Dataset=='FIA'|Dataset=='NWCA'|Dataset=='VNHP', PctCov, PctCov_100))
```
...
DataBase <- DataBase %>% 
  filter(PctCov!=0) #315 observations with zero percent cover
...
DataBase %>% filter(PctCov > 100) %>% nrow() # 453 observations in which PctCover for a taxon per plot is >100%
...
-   checks if all recorded strata == Y have number of strata associated to them

```{r}
DataBase %>% 
  group_by(RecordedStrata) %>% 
  count(RecordedStrata)

DataBase %>% 
  group_by(RecordedStrata) %>% 
  count(NumberOfStrata)

DataBase %>% 
  filter(RecordedStrata=="Y" & is.na(NumberOfStrata)) %>% 
  group_by(Dataset) %>% 
  count() # only NPS
```
...
# Checks whether a plot has only on Site associated to it

```{r}
DataBase %>% select(Site, Plot) %>% distinct() %>% group_by(Plot) %>% summarise(n = n()) %>% filter(n>1) %>% nrow() # 249 plots have different site associated

MultipleSites<-DataBase %>% select(Site, Plot) %>% distinct() %>% group_by(Plot) %>% mutate(n = n()) %>% ungroup() %>% filter(n>1) %>% select(-n) 
MultipleSites2<-DataBase %>% select(Site, Plot) %>% distinct() %>% group_by(Plot) %>% mutate(n = n()) %>% ungroup() %>% filter(n>1) %>% select(-n) %>% left_join(MultipleSites, by="Plot") %>% mutate(id=ifelse(Site.x==Site.y, 1, 0)) %>% filter(id==0 | is.na(id)) %>% group_by(Plot) %>%
 slice_head(n = 1) %>% mutate(SiteNEW=ifelse(is.na(Site.y), Site.x, paste0(Site.x, "; ", Site.y))) %>% select(Plot, SiteNEW)

DataBase <- DataBase %>% left_join(MultipleSites2, by="Plot") %>% mutate(Site=ifelse(is.na(SiteNEW), Site, SiteNEW)) %>% select(-SiteNEW)
nrow(DataBase)
```
...
ecoreg1 <- st_read("/home/shares/neon-inv/data_paper/env_code_data/gis_layers/ecoregionlevel1/NA_CEC_Eco_Level1.shp") %>% rename(EcoRegionLevelI = NA_L1NAME)
glimpse(ecoreg1)
```

Adding Ecoregion Level I as a column

```{r}

DataBaseMissing <- DataBase %>% filter(FuzzedCoord == "Missing") %>% mutate(NA_L1CODE=NA, EcoRegionLevelI=NA, NA_L1KEY=NA, Shape_Leng=NA, Shape_Area=NA)
...
# Cheking differences in number of plots, resampled plots and observations since the last version

```{r}
# imports last version
FULLDatabase_05272022 <- readRDS("/home/shares/neon-inv/data_paper/final_data/archived/FULLDatabase_05272022.rds")

# number of records #
dim(FULLDatabase)
dim(FULLDatabase_05272022) 
...
```{r echo=TRUE}
write_rds(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_09232022.rds")
write_csv(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_09232022.csv")
```
...
DesiredColumns2 <- c("Dataset", "Site", "Original.Site",
                    "Plot", "Original.Plot", "Long", "Lat","FuzzedCoord", "Zone", "EcoRegionLevelI",
                    "Year", "Month", "SpCode", "AcceptedTaxonName", "Original.TaxonName", 
                    "NativeStatus", "Duration", "GrowthHabit",
                    "PctCov", "PctCov_100", "RecordedStrata", "NumberOfStrata", "PlotArea.m2", "SamplingMethod", "Resampled"
                    )
...
# write.csv(ListRareSpecies, "/home/shares/neon-inv/data_paper/final_data/ListRareSpecies_09202022.csv", row.names = F)
```
...
SPCIS_plant_taxa <- SPCISDatabase %>% 
  select(Plot, Year, SpCode, AcceptedTaxonName, Original.TaxonName, 
         NativeStatus, Duration, GrowthHabit, PctCov, PctCov_100, RecordedStrata, NumberOfStrata) %>% 
  as_tibble()

SPCIS_plots <- SPCISDatabase %>% 
  select(Dataset, Plot, Year, Month, Original.Plot, Site, Original.Site, Long, Lat, FuzzedCoord, Zone, 
         EcoRegionLevelI, PlotArea.m2, SamplingMethod, Resampled) %>%
...
#all rows are exactly the same and showing in both dataframes when I search for them. I think it should be ok.
...
```{r echo=TRUE}
#plant data
write_rds(SPCIS_plant_taxa, "/home/shares/neon-inv/data_paper/final_data/SPCIS_plant_taxa_09232022.rds")
write_csv(SPCIS_plant_taxa, "/home/shares/neon-inv/data_paper/final_data/SPCIS_plant_taxa_09232022.csv")

#plot data
write_rds(SPCIS_plots, "/home/shares/neon-inv/data_paper/final_data/SPCIS_plots_09232022.rds")
write_csv(SPCIS_plots, "/home/shares/neon-inv/data_paper/final_data/SPCIS_plots_09232022.csv")
```
...
ggsave("/home/shares/neon-inv/data_paper/figures/GraphInvadedPlotsperDataset09232022.png", dpi=300)
```
...
## months
table(SPCISDatabase$Month, useNA = "ifany")
MissingMonth<-SPCISDatabase %>% 
  filter(is.na(Month)) %>% 
  count(Dataset, Site, Year)

SPCISDatabase %>% #of obs missing per dataset
  filter(is.na(Month)) %>% 
  count(Dataset)
...
## number of times a plot was resampled per data set
SPCISDatabase %>%
  ungroup() %>%
...
SPCISDatabaseEco <- SPCISDatabase %>% 
  filter(FuzzedCoord != "Missing") %>% 
  group_by(Dataset, Lat, Long, Plot, Year, NativeStatus, EcoRegionLevelI) %>% 
...
SPCISDatabaseEco %>% select(EcoRegionLevelI, Dataset) %>% distinct() %>% group_by(EcoRegionLevelI) %>% count() %>% filter(n>1)
```
...
# SPCISDatabase %>% filter(NativeStatus == status,
#                      !is.na(EcoRegionLevelI)) %>%  
...
graph3I<-SPCISDatabaseEco %>% filter(NativeStatus == status,
                    EcoRegionLevelI == "MARINE WEST COAST FOREST"
                     ) %>%  
  ggplot(aes(PctCov_100, fill=Dataset), alpha=0.5) + geom_histogram(binwidth = 10) + 
...
graph4I<-SPCISDatabaseEco %>% filter(NativeStatus == status,
                    EcoRegionLevelI == "MEDITERRANEAN CALIFORNIA"
                     ) %>%  
  ggplot(aes(PctCov_100, fill=Dataset), alpha=0.5) + geom_histogram(binwidth = 10) + 
...
graph5I<-SPCISDatabaseEco %>% filter(NativeStatus == status,
                    EcoRegionLevelI == "NORTH AMERICAN DESERTS"
                     ) %>%  
  ggplot(aes(PctCov_100, fill=Dataset), alpha=0.5) + geom_histogram(binwidth = 10) + 
  scale_fill_manual(values=c(AIMc, FIAc, NEONc, NPSc, NWCAc)) +
  facet_wrap(~Dataset, scales="free_y") +
...
graph6I<-SPCISDatabaseEco %>% filter(NativeStatus == status,
                    EcoRegionLevelI == "NORTHERN FORESTS"
                     ) %>%  
  ggplot(aes(PctCov_100, fill=Dataset), alpha=0.5) + geom_histogram(binwidth = 10) + 
...
graph7I<-SPCISDatabaseEco %>% filter(NativeStatus == status,
                    EcoRegionLevelI == "NORTHWESTERN FORESTED MOUNTAINS"
                     ) %>%  
  ggplot(aes(PctCov_100, fill=Dataset), alpha=0.5) + geom_histogram(binwidth = 10) + 
  scale_fill_manual(values=c(AIMc, FIAc, NEONc, NPSc, NWCAc)) +
  facet_wrap(~Dataset, scales="free_y")  + 
...
graph8I<-SPCISDatabaseEco %>% filter(NativeStatus == status,
                    EcoRegionLevelI == "SOUTHERN SEMIARID HIGHLANDS"
                     ) %>%  
  ggplot(aes(PctCov_100, fill=Dataset), alpha=0.5) + geom_histogram(binwidth = 10) + 
  scale_fill_manual(values=c(AIMc, NEONc, NPSc, NWCAc)) +
  facet_wrap(~Dataset, scales="free_y") +
...
graph9I<-SPCISDatabaseEco %>% filter(NativeStatus == status,
                    EcoRegionLevelI == "TEMPERATE SIERRAS"
                     ) %>%  
  ggplot(aes(PctCov_100, fill=Dataset), alpha=0.5) + geom_histogram(binwidth = 10) + 
...
png('/home/shares/neon-inv/data_paper/figures/ecoregionsNN/graphcombinedEco1_2-09232022.png', width = 1920,height = 1080)
print(all1)
dev.off()
...
png('/home/shares/neon-inv/data_paper/figures/ecoregionsNN/graphcombinedEco3_4-09232022.png', width = 1920,height = 1080)
print(all1)
dev.off()
...
png('/home/shares/neon-inv/data_paper/figures/ecoregionsNN/graphcombinedEco5_6-09232022.png', width = 1920,height = 1080)
print(all1)
dev.off()
...
png('/home/shares/neon-inv/data_paper/figures/ecoregionsNN/graphcombinedEco7_8-09232022.png', width = 1920,height = 1080)
print(all1)
dev.off()
...
png('/home/shares/neon-inv/data_paper/figures/ecoregionsNN/graphcombinedEco9-09232022.png', width = 1920,height = 1080)
print(all1)
dev.off()
