...
Imports the file
```{r}
FIA <- read_csv("/home/shares/neon-inv/data_paper/data_by_dataset/FIA Veg data 12-21-21.csv") #ALL data including repeated samples (3207 plots)
glimpse(FIA)
```
...
Resample <- aggregate(INVYR ~ PLOT+STATECD+LAT+LON, data=unique(FIA[c("PLOT","INVYR", "STATECD", "LAT", "LON")]), FUN=length)
...
### [ASKED IAN FOR HELP] Fixing species with real missing status
```{r}
MissingNativeStatusFixed <- read_excel("/home/shares/neon-inv/data_paper/code_by_dataset/taxonomy/MissingNativeStatusMod.xlsx") %>% 
  rename(AcceptedSpeciesNameFIX = AcceptedSpeciesName)

DataBase.fill <- DataBase.fill %>% 
  #supposed to join by SpCode
  left_join(MissingNativeStatusFixed, by = "SpCode") %>% 
  mutate(AcceptedSpeciesName = ifelse(is.na(AcceptedSpeciesName), AcceptedSpeciesNameFIX, AcceptedSpeciesName),
         NativeStatus = ifelse(is.na(NativeStatus) & Zone == "L48", L48,
                               ifelse(is.na(NativeStatus) & Zone == "AK", AK, 
                                      ifelse(is.na(NativeStatus) & Zone == "PR", PR, 
                                             ifelse(is.na(NativeStatus) & Zone == "MissingCoords", L48, NativeStatus))))) %>% 
  select(-PR, -AK, -L48)
...
```{r eval=FALSE, include=FALSE}
write_rds(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_12232021.rds")
write_csv(FULLDatabase, "/home/shares/neon-inv/data_paper/final_data/FULLDatabase_12232021.csv")
```
...
```{r eval=FALSE, include=FALSE}
write_rds(UDIPADatabase, "/home/shares/neon-inv/data_paper/final_data/UDIPADatabase_12202321.rds")
write_csv(UDIPADatabase, "/home/shares/neon-inv/data_paper/final_data/UDIPADatabase_12202321.csv")
```
...
```{r}
UDIPADatabaseEco <- UDIPADatabase %>% 
  filter(Zone != "MissingCoords") %>% 
...
UDIPADatabaseEco %>% filter(NativeStatus == status,
                     !is.na(Ecoregion)) %>%  
...
table(UDIPADatabaseEco$Ecoregion)

# EASTERN TEMPERATE FORESTS
graph1<-UDIPADatabaseEco %>% filter(NativeStatus == status,
...
# GREAT PLAINS
graph2<-UDIPADatabaseEco %>% filter(NativeStatus == status,
...
graph3<-UDIPADatabaseEco %>% filter(NativeStatus == status,
...
graph4<-UDIPADatabaseEco %>% filter(NativeStatus == status,
...
graph5<-UDIPADatabaseEco %>% filter(NativeStatus == status,
...
# NORTHERN FORESTS
graph6<-UDIPADatabaseEco %>% filter(NativeStatus == status,
...
graph7<-UDIPADatabaseEco %>% filter(NativeStatus == status,
...
graph8<-UDIPADatabaseEco %>% filter(NativeStatus == status,
...
# TEMPERATE SIERRAS
graph9<-UDIPADatabaseEco %>% filter(NativeStatus == status,
