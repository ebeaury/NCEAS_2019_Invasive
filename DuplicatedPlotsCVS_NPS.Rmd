...
WVNHPNPSDuplicatedPlots <- WVNHPPlots %>% left_join(NPSPlots, by=c("Site","Plot")) %>% filter(!is.na(UniqueID.y)) %>% rename(UniqueID_WVNHP = UniqueID.x, UniqueID_NPS = UniqueID.y) %>% select(UniqueID_WVNHP,UniqueID_NPS) # yup! 135 duplicates.
