...
# All species cover data from BLM AIM&LMF protocols, through 2016:
AllSp <- read_rds("AIM_AllSpTax_14Nov2019.rds")
glimpse(AllSp)

##  Make format match desired for NCEAS AI:
AllSp <- AllSp %>%
  rename(Plot = PLOTKEY, Long = NAD83.X, Lat = NAD83.Y, Year = VisitYear,
         Species = bestname, Exotic = inv_L48) %>%
  mutate(Pct_Cov = prop.cover * 100) %>%
  dplyr::select(-code, -genus, -prop.cover)
head(AllSp)
...
AIM.Sp <- AllSp %>%
  ...
  unite(PlotByStatus, Plot, Year, Exotic, sep = '_') 
...
codynMetrics <- H.df %>%
  ...
  inner_join(Even.df) %>%
  separate(PlotByStatus, c("Plot", "Year", "Exotic"), sep = "_") %>%
...
write_csv(AIM.PlotDiv, "AIM_PlotDiv_2Jan2020.csv")
