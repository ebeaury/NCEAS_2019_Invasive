...
## checking for duplicates of multiple months per year of data collection

IL_CTAPMultipleMonth<-IL_CTAP %>% 
  select(SiteID, Transect, PYear, PMonth) %>% 
  distinct() %>% 
  group_by(SiteID, Transect, PYear) %>% 
  summarize(n_obs = n()) %>% 
  filter(n_obs > 1) %>% # yes there are 25 instances
  ungroup()
...
```{r}
IL_CTAP2 %>% write.csv(file.path(data_output, "IL_CTAP_DataPaper09122022.csv"))
```
