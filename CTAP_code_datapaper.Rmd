...
wet <- w %>% 
  ...
  mutate(SampledArea = .25, 
         PctCov = mean(PctCov)/SampledArea, 
         code = 0, 
         layer = "herbaceous") %>% 
  distinct() %>% 
  ungroup() %>% 
...
wettree <- wtree %>% 
  ...
  filter(OriginalSpeciesName != "None") %>% 
  group_by(SiteID, PYear, OriginalSpeciesName) %>%
  rename(SampledArea = Squmetr) %>% 
  mutate(PctCov = sum(dbh)/SampledArea, 
         code = 1,
         layer = "tree") %>% 
  select(-dbh) %>% 
  distinct() %>% 
...
IL_CTAP2 <- left_join(IL_CTAP2, latlong, by = c("SiteID", "PYear"))
glimpse(IL_CTAP2)
```
...
```{r}
IL_CTAP2 %>% write.csv(file.path(data_output, "IL_CTAP_DataPaper10262021.csv"))
```
