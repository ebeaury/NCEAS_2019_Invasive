...
IL_CTAP <- forestfinal %>% 
  ...
  mutate(Dataset = "IL_CTAP",
         SampledArea = 0.25) %>% 
  #
  group_by(Dataset, SiteID, Transect, PYear, OriginalSpeciesName, 
           layer, RecordedStrata, SampledArea, PlotArea.m2) %>% 
  ...
  dplyr::summarise(PctCov = sum(PctCov),
                   ...
                   SampledAreaOriginal = dplyr::first(SampledAreaOriginal)) %>% 
  ungroup() %>% 
  distinct() %>% 
  #
...
IL_CTAP2 <- IL_CTAP2 %>% 
  ...
  group_by(Dataset, SiteID, Transect, PYear, 
           RecordedStrata, SampledArea, PlotArea.m2,
           Accepted.Symbol, inv_L48, Lat, Long, Plot) %>% 
  # this is the column I need to sum because trees present in different strata had distinct area sampled, so they are different observations in the data.
  dplyr::summarise(PctCov = sum(PctCov),
                   # different trees were sampled in the different sampled area, I am keeping the first one
                   SampledAreaOriginal = which.max(SampledAreaOriginal),
...
```{r}
IL_CTAP2 %>% write.csv(file.path(data_output, "IL_CTAP_DataPaper04082022.csv"))
```
